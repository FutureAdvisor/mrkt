  ...
  describe '#authenticate!' do
    ...
    context 'when the token has expired and @retry_authentication = true' do
      ...
      let(:expired_authentication_stub) do
        { :access_token => UUID.generate, :token_type => 'bearer', :expires_in => 0, :scope => 'RestClient' }
      end

      let(:valid_authentication_stub) do
        { :access_token => UUID.generate, :token_type => 'bearer', :expires_in => 1234, :scope => 'RestClient' }
      end
